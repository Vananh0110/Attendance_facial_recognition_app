...
const getClassesForStudent = async (req, res) => {
  ...
  try {
    const results = await pool.query(queries.getClassesForStudentQuery, [
      userId,
    ]);
...
const getStudentsInClass = async (req, res) => {
  ...
  try {
    const results = await pool.query(queries.getStudentsInClassQuery, [
      classId,
    ]);
    res.status(200).json(results.rows);
  } catch (error) {
...
const addStudentToClass = async (req, res) => {
  ...
  try {
    const result = await pool.query(queries.addStudentToClassQuery, [
      student_id,
      class_id,
    ]);
